...
const CHART_PALETTES = {
    ...
    ubuntu: {
        cpu: { stroke: '#e95420', fillTop: 'rgba(233, 84, 32, 0.35)', fillBottom: 'rgba(233, 84, 32, 0.0)' },
        gpu: { stroke: '#9e3389', fillTop: 'rgba(158, 51, 137, 0.35)', fillBottom: 'rgba(158, 51, 137, 0.0)' },
    },
};
...
function showClearLogsModal() {
    ...
    clearLogsPending = false;
    modal.classList.remove('hidden');
    const cancelBtn = document.getElementById('clear-logs-cancel');
    if (cancelBtn) cancelBtn.focus();
}

function handleClearLogsResponse(result) {
    hideClearLogsModal();
    lastSummaryStats = null;
    const summaryModal = document.getElementById('summary-modal');
    if (summaryModal) summaryModal.classList.add('hidden');
    if (!result || result.ok === false) {
        showToast(tr('toast_logs_clear_failed', 'Nie udało się usunąć logów'), 'warning');
        return;
    }
...
function setupClearLogsModal() {
    const modal = document.getElementById('clear-logs-modal');
    if (!modal) return;

    const openBtn = document.getElementById('menu-clear-logs-btn');
    const cancelBtn = document.getElementById('clear-logs-cancel');
    const dismissBtn = document.getElementById('clear-logs-dismiss');
    const confirmBtn = document.getElementById('clear-logs-confirm');
    ...
    if (cancelBtn) cancelBtn.addEventListener('click', hideClearLogsModal);
    if (dismissBtn) dismissBtn.addEventListener('click', hideClearLogsModal);
    modal.addEventListener('click', (e) => {
        if (e.target === modal) hideClearLogsModal();
    });
    if (confirmBtn) {
        confirmBtn.addEventListener('click', () => {
            if (clearLogsPending) return;
            clearLogsPending = true;
            confirmBtn.disabled = true;
            if (api.clearLogs) {
                api.clearLogs();
            } else {
                handleClearLogsResponse({ ok: false, deleted: 0, error: 'unavailable' });
            }
        });
    }
...
function handleCloseRequest() {
    ...
    if (isModalVisible('close-confirm-modal') || isModalVisible('close-manual-modal') || isModalVisible('clear-logs-modal')) return;
...
let summaryMode = 'all';
let summaryDayIndex = 0;

function tr(key, fallback) {
    return (currentTranslations && currentTranslations[key]) || fallback;
}

function getSummaryDays(stats) {
    return Array.isArray(stats && stats.days) ? stats.days : [];
}

function uiLocale() {
    const lang = (document.getElementById('language-select') || {}).value || 'pl';
    return { pl: 'pl-PL', en: 'en-GB', de: 'de-DE', es: 'es-ES', cs: 'cs-CZ' }[lang] || lang;
}
...
function syncSummaryTitle() {
    const el = document.getElementById('summary-modal-title');
    if (!el) return;
...
function currentViewStats(stats) {
    ...
    if (summaryMode !== 'day') return stats;
    if (stats.error) return stats;
    const days = getSummaryDays(stats);
    if (!days.length) {
        return { error: tr('daily_report_empty', 'Brak danych dziennych w logach.') };
    }
    if (summaryDayIndex < 0 || summaryDayIndex >= days.length) {
        summaryDayIndex = days.length - 1;
    }
    return days[summaryDayIndex];
}

function syncSummaryDayNav(stats) {
    const nav = document.getElementById('summary-day-nav');
    if (!nav) return;
    const days = getSummaryDays(stats);
    const daily = summaryMode === 'day' && !stats.error && days.length > 0;
    nav.classList.toggle('hidden', !daily);
    if (!daily) return;
    const prev = document.getElementById('summary-day-prev');
    const next = document.getElementById('summary-day-next');
    const label = document.getElementById('summary-day-label');
    const indexEl = document.getElementById('summary-day-index');
    if (prev) prev.disabled = days.length === 0 || summaryDayIndex <= 0;
    if (next) next.disabled = days.length === 0 || summaryDayIndex >= days.length - 1;
    const day = days[summaryDayIndex];
    if (label) label.textContent = day ? formatDayLabel(day.date) : '—';
    if (indexEl) {
        indexEl.textContent = days.length
            ? tr('daily_report_day_of', '{current} / {total}')
                .replace('{current}', String(summaryDayIndex + 1))
...
function renderLogSummaryModal(stats, options = {}) {
    ...
    const sampleInterval = tr('summary_sample_interval', '(read every ~{sec}s)')
        .replace('{sec}', view.sample_interval_sec);
...
• ${tr('summary_report_zero_rpm', 'Silent Zero-RPM mode (0%):')} ${formatSummaryDuration(g.zero_rpm_sec)} (${g.zero_rpm_pct}%)`;
    ...
    grid.appendChild(summaryCard('general', `ℹ️ ${tr('summary_session_title', 'GENERAL SESSION PARAMETERS')}`, [
        summaryRow(tr('summary_runtime', 'Program runtime:'), formatSummaryDuration(view.total_runtime_seconds)),
        summaryRow(tr('summary_samples', 'Measurement samples:'), `${view.total_samples} ${sampleInterval}`),
        summaryRow(tr('summary_log_range', 'Log time range:'), `${view.start_time} — ${view.end_time}`),
    ], 'grid-column: 1 / -1;'));
...
function updateUI(data) {
    ...
    if (data.clear_logs_response) {
        handleClearLogsResponse(data.clear_logs_response);
        return;
    }
...
function setupEventListeners() {
    ...
    if (settingsSupportBtn) {
        settingsSupportBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            closeAllStatusDropdowns();
            const url = settingsSupportBtn.dataset.url;
            if (url && api.openExternal) api.openExternal(url);
        });
    }
    ...
    const requestLogSummary = (mode) => {
        summaryMode = mode === 'day' ? 'day' : 'all';
        if (summaryMode === 'day') summaryDayIndex = -1;
        closeAllStatusDropdowns();
        const content = document.getElementById('summary-modal-content');
        if (content) {
    ...
    if (menuSummaryBtn) {
        menuSummaryBtn.addEventListener('click', () => requestLogSummary('all'));
    }
    const menuDailyReportBtn = document.getElementById('menu-daily-report-btn');
    if (menuDailyReportBtn) {
        menuDailyReportBtn.addEventListener('click', () => requestLogSummary('day'));
    }

    const shiftSummaryDay = (delta) => {
        if (summaryMode !== 'day' || !lastSummaryStats) return;
        const days = getSummaryDays(lastSummaryStats);
        if (!days.length) return;
        const next = Math.max(0, Math.min(days.length - 1, summaryDayIndex + delta));
        if (next === summaryDayIndex) return;
        summaryDayIndex = next;
        renderLogSummaryModal(lastSummaryStats, { show: false });
    };
    const summaryDayPrev = document.getElementById('summary-day-prev');
    const summaryDayNext = document.getElementById('summary-day-next');
    if (summaryDayPrev) summaryDayPrev.addEventListener('click', () => shiftSummaryDay(-1));
    if (summaryDayNext) summaryDayNext.addEventListener('click', () => shiftSummaryDay(1));
    document.addEventListener('keydown', (e) => {
        if (!summaryModal || summaryModal.classList.contains('hidden') || summaryMode !== 'day') return;
        if (e.key === 'ArrowLeft') {
            e.preventDefault();
            shiftSummaryDay(-1);
...
function applyTranslations() {
    ...
    setText('menu-daily-report-label', 'menu_daily_report');
    setText('menu-open-logs-label', 'menu_open_logs_label');
    setText('menu-clear-logs-label', 'settings_clear_logs');
    setText('settings-btn-label', 'settings_btn_label');
    setText('settings-header-title', 'settings_header_title');
    setText('settings-window-group-title', 'settings_group_window');
    setText('settings-advanced-group-title', 'settings_group_advanced');
    setText('settings-about-group-title', 'settings_group_about');
    setText('settings-support-label', 'settings_support');
    setText('license-support-btn-label', 'license_support_btn');
    ...
    setText('clear-logs-title', 'clear_logs_modal_title');
    ...
    syncSettingsCloseSelect();
    syncSummaryTitle();
    const summaryDayPrevBtn = document.getElementById('summary-day-prev');
    const summaryDayNextBtn = document.getElementById('summary-day-next');
    if (summaryDayPrevBtn) {
        const prevLabel = currentTranslations['daily_report_prev'] || 'Poprzedni dzień';
        summaryDayPrevBtn.setAttribute('aria-label', prevLabel);
        summaryDayPrevBtn.title = prevLabel;
    }
    if (summaryDayNextBtn) {
        const nextLabel = currentTranslations['daily_report_next'] || 'Następny dzień';
        summaryDayNextBtn.setAttribute('aria-label', nextLabel);
        summaryDayNextBtn.title = nextLabel;
    }
    const summaryModalOpen = document.getElementById('summary-modal');
    if (lastSummaryStats && summaryModalOpen && !summaryModalOpen.classList.contains('hidden')) {
        renderLogSummaryModal(lastSummaryStats, { show: false });
    }
