...
def _local_date_key(ts: datetime) -> str:
    """Calendar day in the local timezone (telemetry timestamps are UTC)."""
    if ts.tzinfo is not None:
        ts = ts.astimezone()
    return ts.date().isoformat()


def _read_log_rows(filepath: Path) -> List[Dict[str, Any]]:
...
def stats_from_rows(rows: List[Dict[str, Any]], filepath: Optional[str] = None) -> Dict[str, Any]:
    if not rows:
        return {"error": "Brak próbek w wybranym zakresie."}
...
def analyze_logs(filepath: Path) -> Dict[str, Any]:
    rows = _read_log_rows(filepath)
    if not rows:
        return {"error": f"Plik logów '{filepath}' jest pusty lub ma nieprawidłowy format."}

    stats = stats_from_rows(rows, str(filepath))
    by_day: Dict[str, List[Dict[str, Any]]] = {}
    for row in rows:
        by_day.setdefault(_local_date_key(row['timestamp']), []).append(row)
    stats["days"] = []
    for date in sorted(by_day):
        day = stats_from_rows(by_day[date], str(filepath))
        day["date"] = date
