...
if systemctl is-active --quiet acer-speedfan.service 2>/dev/null \
   || [ -f /etc/systemd/system/acer-speedfan.service ]; then
    warn "została poprzednia usługa acer-speedfan.service — sudo ./install.sh ją zastąpi"
fi
echo
...
if systemctl list-unit-files nitro-fan-daemon.service >/dev/null 2>&1 \
   && systemctl is-active --quiet nitro-fan-daemon.service 2>/dev/null; then
    warn "stara usługa nitro-fan-daemon.service nadal działa — wyłącz ją"
fi
echo
