...
Fan control for **Acer Nitro 5** on **Linux + systemd** — a background daemon and an Electron dashboard.
...
`setup.sh` checks your laptop, installs the packages, fan driver, background service and the GUI. Done — the service starts at boot.

- **Will it run on my laptop?** `./check-system.sh` — read-only, ends with a plain **YES / MAYBE / NO**.
- **Step by step:** [INSTALL.md](INSTALL.md) · **Po polsku:** [INSTALL_PL.md](INSTALL_PL.md) · [README_PL.md](README_PL.md)

> **Safety.** Manual fan control can **overheat and damage hardware**. CPU PWM is **hard-clamped to 30%**. **Use at your own risk.**

## Features

- Cooling profiles **Silent / Balanced / Turbo** and a custom temperature→speed curve editor with live preview
- Manual PWM sliders (CPU floor 30%), GPU Zero-RPM, EMA smoothing — set it once, the daemon keeps it, no window needed
...
MIT — [LICENSE](LICENSE). © 2024 [keizenx](https://github.com/keizenx), © 2026 [Kaspral1](https://github.com/Kaspral1). GUI lineage: [keizenx/nitro-fan-control](https://github.com/keizenx/nitro-fan-control). Optional parts: the `acer-nitro-ec` kernel module is GPL-2.0; DAMX is GPL-3.0 and **not** bundled — [THIRD_PARTY.md](THIRD_PARTY.md).
